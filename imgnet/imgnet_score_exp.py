...
parser.add_argument('--save',           type=str,   default='experiments/imgnet_score', help="define the save directory")
...
for seed in unique_seeds:
    ...
    # perturb test scores
    test_scores_corrupt = tst_scores[np.arange(tst_scores.shape[0]), tst_labels]
    test_scores_corrupt = perturb_test_scores(test_scores_corrupt, 
                                              corrupt_ratio=args.corrupt_ratio, 
                                              noise_upper=args.noise_upper, 
                                              noise_lower=args.noise_lower)
    # assign perturbed scores back
    tst_scores[np.arange(tst_scores.shape[0]), tst_labels] = test_scores_corrupt
    tst_scores = tst_scores.numpy()
...
plot_cp(coverage_results, plt_type='Coverage', plt_name=f'imgnet_{args.corrupt_ratio}_{args.noise_upper}_{args.noise_lower}_cover.png', save_dir='score_figures')
plot_cp(size_results, plt_type='Size', plt_name=f'imgnet_{args.corrupt_ratio}_{args.noise_upper}_{args.noise_lower}_size.png', save_dir='score_figures')
