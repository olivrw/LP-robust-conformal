    ...
    *,
    ...
    for i, (grp, lbl, col) in enumerate(zip(data_arr, group_labels, colours)):
        ...
        ax.scatter(
            ...
            marker=marker,
            color=col,
            alpha=0.65 if col == highlight_color else 0.90,
            edgecolor="white",
            linewidth=0.5,
        ...
        ax.hlines(
            ...
            xmax=x_c + 0.35,
            color=col,
            linewidth=3 if col == highlight_color else 2,
            zorder=4,
        )
    ...
    legend_handles = [
        Line2D([0], [0], marker="^", color="w", label=r"$\mathbf{LP}_{\boldsymbol{\epsilon}}$ & $\mathbf{LP}^{\mathbf{est}}_{\boldsymbol{\epsilon}}$",
               markerfacecolor=highlight_color, markersize=12),
        Line2D([0], [0], marker="o", color="w", label="Other methods",
               markerfacecolor=other_color, markersize=12),
    ]
    ax.legend(handles=legend_handles, loc="upper left", frameon=False,
              fontsize=11, handlelength=1.5, borderpad=0.6)
...
reg_results = pd.read_csv('../experiments/mnist/robust_to_both/reg_result_hist_0.01_0.25_-0.25.csv').to_numpy()
coverage_results = [reg_results[:, i] for i in range(1, 8)]
size_results = [reg_results[:, j] for j in range(8, 15)]

plot_cp(coverage_results, plt_type='Coverage', plt_name=f'mnist_{0.01}_{0.25}_{-0.25}_cover.png',
        save_dir='figures', )
plot_cp(size_results, plt_type='Size', plt_name=f'mnist_{0.01}_{0.25}_{-0.25}_size.png', save_dir='figures', ylims=None)


reg_results = pd.read_csv('../experiments/mnist/robust_to_both/reg_result_hist_0.025_0.5_-0.5.csv').to_numpy()
coverage_results = [reg_results[:, i] for i in range(1, 8)]
size_results = [reg_results[:, j] for j in range(8, 15)]

plot_cp(coverage_results, plt_type='Coverage', plt_name=f'mnist_{0.025}_{0.5}_{-0.5}_cover.png',
        save_dir='figures', )
plot_cp(size_results, plt_type='Size', plt_name=f'mnist_{0.025}_{0.5}_{-0.5}_size.png', save_dir='figures', ylims=None)


reg_results = pd.read_csv('../experiments/mnist/robust_to_both/reg_result_hist_0.05_1.0_-1.0.csv').to_numpy()
coverage_results = [reg_results[:, i] for i in range(1, 8)]
size_results = [reg_results[:, j] for j in range(8, 15)]

plot_cp(coverage_results, plt_type='Coverage', plt_name=f'mnist_{0.05}_{1.0}_{-1.0}_cover.png',
        save_dir='figures', )
plot_cp(size_results, plt_type='Size', plt_name=f'mnist_{0.05}_{1.0}_{-1.0}_size.png', save_dir='figures', ylims=None)
